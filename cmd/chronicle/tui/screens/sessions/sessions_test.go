...
func TestInit_ReturnsLoadCommand(t *testing.T) {
	...
	// Init batches the fetch with the spinner's tick command,
	// so the outer call resolves to a tea.BatchMsg. The fetch
	// is one of the inner commands; iterate the batch and find
	// the loadedMsg the test cares about.
...
// findLoadedMsg walks the batch of commands Init returns and
// runs each one until it finds the loadedMsg the test asserts
// against. The spinner's tick command resolves to a different
// type, so the helper skips past it without failing.
