...
// today) contribute nothing. The function returns an empty
// slice (not an error) when no provider has memory data,
// which is the normal state for a fresh install.
//
// The CLI uses this for `chronicle memory list`. The same
