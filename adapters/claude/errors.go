...
// "claude: list" instead of just "list", which makes a multi-
...
type Error struct {
	...
	// Path is the relative path inside the Claude root that
...
// "claude: <op>: <path>: <err>" with the path piece omitted when
...
func (e *Error) Error() string {
	...
	case e.Err == nil:
		return fmt.Sprintf("claude: %s: %s", e.Op, e.Path)
	default:
		return fmt.Sprintf("claude: %s: %s: %v", e.Op, e.Path, e.Err)
	}
}
